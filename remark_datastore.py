...
def ReadRemarks(user_id):
  ...
  remarks = []
  colors = ['AliceBlue','AntiqueWhite ','Aqua','Chartreuse','Brown','Gray','Purple','Orange','Blue','LightBlue']
  rand_num = randInt(len(colors))
  ...
  for remark in query.fetch():
    ...
    remarks.append((remark.user, remark.text, 'colors[rand_num]'))
